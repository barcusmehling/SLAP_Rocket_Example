...
base_accel_locs = []; % initialize empty matrix for accel locs on outer diameter of baseplate
for jj = 1:length(ths)
    base_accel_locs(jj,2:4) = [r*cosd(ths(jj)) r*sind(ths(jj)) z];
end

% assemble a few more potential accel locations in the center of the baseplate
vec1 = [-1.2 0 1.2];
base_accel_locs2 = [];
c = 1;
